...
const ROADS = (typeof window !== "undefined") ? window.__BGS_DERIVED_ROADS__ : null;
if (!CORE || typeof CORE.hazardChoose !== "function" || !ROADS || typeof ROADS.buildFeatures !== "function") return;

const VERSION = "XGB_RESIDUAL_BIAS_V1_17D_ROADS";
...
const FEATURE_NAMES = [...BASE_FEATURE_NAMES, ...ROADS.featureNames];
...
function buildFeatures(seq, corePrediction, signal = null) {
  ...
  const roadFeatures = ROADS.buildFeatures(seq);
  return {
    core_p_b: corePB,
...
function renderPrediction(p, historyLength) {
  ...
  if (el("modePill")) el("modePill").textContent = p.residualBias?.active ? "XGB 17D 修正完成" : "分析完成";
...
function downloadTrainingData() {
  ...
  a.href = url;
  a.download = `bgs_xgb_residual_17d_training_${Date.now()}.json`;
...
if (typeof window !== "undefined") {
  window.__BGS_RESIDUAL_BIAS__ = {
    ...
    getDerivedRoadFeatures: seq => ROADS.buildFeatures(seq),
    getModelStatus: () => ({
      loaded: modelLoaded,
      trained: Boolean(modelBundle?.trained),
      featureCount: FEATURE_NAMES.length,
      featureSchema: "7D_CORE_PLUS_10D_DERIVED_ROADS",
