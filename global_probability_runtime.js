...
const VERSION="PHYSICS_56D_GLOBAL_PROB_V1";
const PHYSICS_URL="physics_multitask_model.json";
const GLOBAL56_URL="global_probability_model.json";
...
const TRAINING_KEY="bgs_xgb_global_training_v3",PENDING_KEY="bgs_xgb_global_pending_v3";
const SHOE_KEY="bgs_xgb_global_shoe_id_v3",CUT_KEY="bgs_xgb_estimated_total_hands_v1";
...
let physicsBundle=null,global56Bundle=null;
let status={physics:false,global56:false,errors:[]};
...
function getEstimatedTotalHands(){
  try{const c=+(window.__BGS_GLOBAL_CONFIG__?.estimatedTotalHands||0);if(c>=40&&c<=90)return c;const s=+localStorage.getItem(CUT_KEY);if(s>=40&&s<=90)return s;}catch(_){}
  return 60;
}
...
function predictGlobalProbability(bundle,vector,names){
...
function dynamicClip(rawPB,extended){
  const remainingRatio=clip(+extended[4]||0);
  const physicsB=clip(+extended[31]||.5),physicsP=clip(+extended[32]||.5);
  const imbalance=Math.min(1,Math.abs(physicsB-physicsP)/.20);
  let width=.055+.050*(1-remainingRatio)+.015*imbalance;
  width=Math.max(.05,Math.min(.12,width));
  return {value:clip(rawPB,.5-width,.5+width),low:.5-width,high:.5+width,width};
}

function applyGlobalPrediction(seq,corePrediction){
  const original7=buildOriginal7(seq,corePrediction),corePB=original7.core_p_b;
  let physics=null,extended=null,rawPB=corePB,finalPB=corePB,clipInfo=null,error="",mode="core";
  try{
    if(physicsBundle?.trained&&global56Bundle?.trained){
      physics=predictPhysics(seq);
      extended=buildExtended(corePB,original7,physics);
      rawPB=predictGlobalProbability(global56Bundle,extended,EXTENDED_NAMES);
      clipInfo=dynamicClip(rawPB,extended);
      finalPB=clipInfo.value;
      mode="global56";
    }
  }catch(e){error=String(e?.message||e||"runtime_error");rawPB=corePB;finalPB=corePB;mode="core";}
  const direction=finalPB>.5?"B":"P",finalPP=1-finalPB;
  return {...corePrediction,direction,confidence:direction==="B"?finalPB:finalPP,probabilities:{B:finalPB,P:finalPP},
    regime:mode==="global56"?(direction!==corePrediction.direction?"Global XGB換邊":"Global XGB裁決"):corePrediction.regime,
    globalProbability:{version:VERSION,active:mode==="global56",mode,corePB,rawPB,finalPB,clip:clipInfo,
...
function registerPrediction(seq,prediction){
  const r=prediction.globalProbability||{},f=r.original7||buildOriginal7(seq,prediction);
...
function settlePending(actualOutcome){
  ...
  const row={schema_version:3,...p,actual_outcome:actual,actual_b:actual==="B"?1:0},rows=readRows();
...
function exportTrainingData(){return JSON.stringify({schema_version:3,target:"actual_B",feature_names:ORIGINAL7_NAMES,rows:readRows()},null,2);}
function downloadTrainingData(){const blob=new Blob([exportTrainingData()],{type:"application/json;charset=utf-8"}),url=URL.createObjectURL(blob),a=document.createElement("a");a.href=url;a.download="bgs_global56_training_"+Date.now()+".json";document.body.appendChild(a);a.click();a.remove();URL.revokeObjectURL(url);}
...
async function loadModels(){
  status={physics:false,global56:false,errors:[]};
  const rs=await Promise.allSettled([fetchBundle(PHYSICS_URL),fetchBundle(GLOBAL56_URL)]);
  ...
  if(rs[1].status==="fulfilled"&&rs[1].value?.model_type==="xgb_global_probability_classifier"){global56Bundle=rs[1].value;status.global56=!!global56Bundle.trained;}else status.errors.push("global56_model");
  return status;
}
...
function renderPrediction(p,n){
  ...
  if(el("modePill"))el("modePill").textContent=p.globalProbability?.mode==="global56"?"Physics Global 56D":"Core";
...
function installUI(){
  ...
  btn.addEventListener("click",()=>{const history=readHistory();if(!history.length){const m=document.getElementById("message");if(m)m.textContent="請先輸入牌局紀錄";return;}
    const core=CORE.hazardChoose(history),p=applyGlobalPrediction(history,core);saveSelection(p.direction);registerPrediction(history,p);renderPrediction(p,history.length);});
...
if(typeof window!=="undefined")window.__BGS_GLOBAL56__={version:VERSION,applyGlobalPrediction,predictPhysics,buildOriginal7,historyVector,loadModels,setEstimatedTotalHands,getEstimatedTotalHands,
  exportTrainingData,downloadTrainingData,getTrainingCount:()=>readRows().length,getModelStatus:()=>({...status,mode:status.physics&&status.global56?"global56":"core"})};
