(() => {
  ...
  const MODEL_VERSION = "FROZEN-V2-128D-MULTI-EXPERT";
  const PRIOR_SOURCE = "engineered_static_multi_expert_no_training_dataset";
  const STORAGE_KEY = "bgs128d_frozen_v2_multi_expert";
  ...
  const sign = v => v > 1e-12 ? 1 : v < -1e-12 ? -1 : 0;
  ...
  function ratioSign(seq, w) { return clip(2 * bankerRatio(seq, w) - 1, -1, 1); }
  ...
  function hazardContexts(side, cur, prev) {
    const ph = prev.length ? prev.at(-1) : 0;
    const deltas = [];
  ...
  function candidateRoadFit(seq, candidate) {
    const before = buildDerivedRoads(seq);
    const after = buildDerivedRoads([...seq, candidate]);
    const names = ["big_eye", "small_road", "cockroach_road"];
    const parts = [];
    const newMarks = [];

    for (const name of names) {
      const prev = before[name].slice(-8);
      const next = after[name];
      if (next.length <= before[name].length) {
        parts.push({ name, fit: 0.5, support: 0, mark: "" });
        continue;
      }

      const mark = next.at(-1);
      newMarks.push(mark);
      if (!prev.length) {
        parts.push({ name, fit: 0.5, support: 0.1, mark });
        continue;
      }

      const pR = (prev.filter(x => x === "R").length + 1.5) / (prev.length + 3);
      const majorityFit = mark === "R" ? pR : 1 - pR;
      const lastFit = prev.at(-1) === mark ? 0.68 : 0.32;
      const fit = clip(0.68 * majorityFit + 0.32 * lastFit);
      parts.push({ name, fit, support: clip(prev.length / 8), mark });
    }

    const weighted = parts.reduce((s, p) => s + p.fit * (0.45 + 0.55 * p.support), 0);
    const denom = parts.reduce((s, p) => s + (0.45 + 0.55 * p.support), 0) || 1;
    let score = weighted / denom;
    if (newMarks.length >= 2) {
      const same = Math.max(
        newMarks.filter(x => x === "R").length,
        newMarks.filter(x => x === "U").length
      ) / newMarks.length;
      score = clip(0.82 * score + 0.18 * same);
    }

    return {
      score,
      support: mean(parts.map(p => p.support)),
      parts
    };
  }

  function derivedCandidateExpert(seq) {
    const B = candidateRoadFit(seq, "B");
    const P = candidateRoadFit(seq, "P");
    return {
      B,
      P,
      score: clip((B.score - P.score) * 2.1, -1, 1),
      confidence: clip((B.support + P.support) / 2)
    };
  }

  function regimeConsensus(seq) {
    const rs = runs(seq), currentRun = rs.length ? rs.at(-1)[1] : 0;
    const tr4 = turnRate(seq, 4), tr8 = turnRate(seq, 8), tr16 = turnRate(seq, 16);
    const hz = hazard(seq), stable = hsmmStable(seq);
    const alt4 = alternatingTail(seq, 4), alt6 = alternatingTail(seq, 6);
    const same4 = sameTail(seq, 4), vol = runVolatility(seq);

    const shortTrend = 0.36 * (1 - tr4) + 0.29 * (1 - hz) + 0.20 * same4 + 0.15 * clip(currentRun / 4);
    const shortRev   = 0.36 * tr4 + 0.32 * hz + 0.20 * alt4 + 0.12 * vol;

    const midTrend = 0.34 * (1 - tr8) + 0.24 * (1 - hz) + 0.24 * stable + 0.18 * same4;
    const midRev   = 0.34 * tr8 + 0.28 * hz + 0.22 * alt6 + 0.16 * (1 - stable);

    const longTrend = 0.40 * (1 - tr16) + 0.30 * stable + 0.18 * (1 - vol) + 0.12 * clip(currentRun / 6);
    const longRev   = 0.40 * tr16 + 0.25 * (1 - stable) + 0.20 * vol + 0.15 * alt6;

    const votes = [shortTrend - shortRev, midTrend - midRev, longTrend - longRev];
    const signs = votes.map(sign);
    const pos = signs.filter(x => x > 0).length, neg = signs.filter(x => x < 0).length;
    const consensus = Math.max(pos, neg) / 3;
    const trendStrength = clip(mean(votes.map(v => Math.max(0, v))) * 2.3);
    const reversalStrength = clip(mean(votes.map(v => Math.max(0, -v))) * 2.3);
    const conflict = 1 - consensus;

    let regime = "mixed";
    if (pos >= 2 && trendStrength > 0.08) regime = "trend";
    else if (neg >= 2 && reversalStrength > 0.08) regime = "reversal";

    return {
      regime, votes, consensus, conflict,
      trendStrength, reversalStrength,
      turnRate4: tr4, turnRate8: tr8, turnRate16: tr16,
      hazard: hz, stable, volatility: vol, currentRun
    };
  }

  function expertSignals(seq) {
    const side = sideSign(seq);
    const r4 = ratioSign(seq, 4), r8 = ratioSign(seq, 8), r16 = ratioSign(seq, 16), r32 = ratioSign(seq, 32);
    const reg = regimeConsensus(seq);
    const derived = derivedCandidateExpert(seq);

    const continueStrength = clip(
      0.30 * (1 - reg.hazard) +
      0.23 * (1 - reg.turnRate8) +
      0.18 * (1 - reg.turnRate16) +
      0.17 * reg.stable +
      0.12 * clip(reg.currentRun / 5)
    );

    const trendBackbone = clip(
      0.48 * side +
      0.24 * r8 +
      0.18 * r16 +
      0.10 * r32,
    ...
    const trend = clip(trendBackbone * (0.40 + 0.60 * continueStrength), -1, 1);

    const reverseStrength = clip(
      0.31 * reg.hazard +
      0.24 * reg.turnRate4 +
      0.20 * reg.turnRate8 +
      0.14 * alternatingTail(seq, 6) +
      0.11 * (1 - reg.stable)
    );
    const shortShift = clip(0.62 * (r4 - r16) + 0.38 * (r8 - r32), -1, 1);
    const reversalDirection = clip(0.64 * (-side) + 0.36 * shortShift, -1, 1);
    const reversal = clip(reversalDirection * (0.38 + 0.62 * reverseStrength), -1, 1);

    const divergence = clip(0.58 * (r4 - r16) + 0.42 * (r8 - r32), -1, 1);

    const rawWeights = {
      trend: 0.30 * (0.62 + 0.78 * reg.trendStrength) * (reg.regime === "trend" ? 1.15 : 1),
      reversal: 0.20 * (0.62 + 0.82 * reg.reversalStrength) * (reg.regime === "reversal" ? 1.12 : 1),
      derived: 0.27 * (0.72 + 0.68 * derived.confidence + 0.40 * Math.abs(derived.score)),
      divergence: 0.23 * (0.76 + 0.55 * Math.abs(divergence))
    };

    if (reg.conflict > 0.30) {
      rawWeights.trend *= 0.78;
      rawWeights.reversal *= 0.78;
      rawWeights.derived *= 1.10;
      rawWeights.divergence *= 1.08;
    }

    const wsum = Object.values(rawWeights).reduce((a, b) => a + b, 0) || 1;
    const weights = Object.fromEntries(Object.entries(rawWeights).map(([k, v]) => [k, v / wsum]));

    const fused = clip(
      weights.trend * trend +
      weights.reversal * reversal +
      weights.derived * derived.score +
      weights.divergence * divergence,
      -1, 1
    );

    return {
      side, ratios: { r4, r8, r16, r32 },
      trend, reversal, divergence, derived,
      weights, regime: reg, fused
    };
  }

  function featureIndex(name) {
    const i = FEATURE_NAMES.indexOf(name);
    if (i < 0) throw new Error(`Unknown feature: ${name}`);
    return i;
  }

  function buildFrozenBasePrior() {
    const thetaB = Array(DIM).fill(0), thetaP = Array(DIM).fill(0), aDiag = Array(DIM).fill(RIDGE);
    const pair = (name, c, precision = RIDGE) => {
    ...
    // Low-amplitude base context only. The multi-expert layer carries the structural direction.
    pair("current_side_banker_binary", 0.025, 7.0);
    pair("current_side_player_binary", -0.025, 7.0);
    pair("recent4_banker_ratio", 0.028, 6.0);
    pair("recent8_banker_ratio", 0.040, 6.0);
    pair("recent16_banker_ratio", 0.028, 6.0);
    pair("recent32_banker_ratio", 0.018, 6.0);
  ...
  const FROZEN_BASE = buildFrozenBasePrior();

  function baseArmScore(arm, x) {
    const theta = arm.b.map((v, i) => v / arm.A_diag[i]);
    let meanScore = 0, quad = 0;
    for (let i = 0; i < x.length; i++) {
      meanScore += x[i] * theta[i];
      quad += (x[i] * x[i]) / arm.A_diag[i];
    }
    return { mean: meanScore, uncertainty: Math.sqrt(Math.max(0, quad)) };
  }

  function contextHealth(raw, seq, experts) {
    const x = normalizeContext(raw);
    const n = bp(seq).length;
    const maturity = clip(n / 24);
    const entropy = binaryEntropyNorm(seq, 12);
    const derivedSupport = experts.derived.confidence;
    const regimeAgreement = experts.regime.consensus;
    const energy = Math.sqrt(mean(x.map(v => v * v)));
    const quality = clip(
      0.22 * maturity +
      0.25 * (1 - 0.65 * entropy) +
      0.25 * derivedSupport +
      0.20 * regimeAgreement +
      0.08 * clip(energy)
    );
    return { x, quality, maturity, entropy, derivedSupport, regimeAgreement, energy };
  }

  function choose(seq) {
    const rawX = context128(seq);
    const experts = expertSignals(seq);
    const health = contextHealth(rawX, seq, experts);
    const baseB = baseArmScore(FROZEN_BASE.B, health.x);
    const baseP = baseArmScore(FROZEN_BASE.P, health.x);
    const baseGap = baseB.mean - baseP.mean;

    // Fused structural score dominates; base 128D linear prior only breaks weak structural ambiguity.
    let directional = clip(
      0.84 * experts.fused +
      0.16 * clip(baseGap * 3.0, -1, 1) +
      0.010,
      -1, 1
    );

    // When regimes conflict, compress rather than flipping every directional feature together.
    if (experts.regime.conflict > 0.30) directional *= 0.86;
    if (health.quality < 0.38) directional *= 0.88;

    let direction, reason;
    if (Math.abs(directional) <= SCORE_TIE_EPS) {
      let h = 2166136261 >>> 0;
      const token = `${MODEL_VERSION}|${seq.join("")}`;
      for (let i = 0; i < token.length; i++) { h ^= token.charCodeAt(i); h = Math.imul(h, 16777619) >>> 0; }
      direction = h & 1 ? "B" : "P";
      reason = "fixed_hash_tie_only";
    } else {
      direction = directional > 0 ? "B" : "P";
      reason = "frozen_v2_multi_expert_argmax";
    }

    const meanB = directional / 2;
    const meanP = -directional / 2;
    const commonUncertainty = (baseB.uncertainty + baseP.uncertainty) / 2;
    const scores = {
      B: { mean: meanB, uncertainty: commonUncertainty, score: meanB },
      P: { mean: meanP, uncertainty: commonUncertainty, score: meanP }
    };
    const gap = scores.B.score - scores.P.score;

    const calibratedGap = gap * (0.60 + 0.40 * health.quality);
    const rawPB = 1 / (1 + Math.exp(-Math.max(-8, Math.min(8, calibratedGap / SCORE_TEMP))));
    const pB = clip(rawPB, PROB_MIN, PROB_MAX), pP = 1 - pB;
    const confidence = direction === "B" ? pB : pP;

    return {
      direction, reason, rawX, x: health.x, experts, health, scores, gap,
  ...
  function addOutcome(outcome) {
    ...
    state.active = false;
    state.lastPrediction = null;
    setMessage(`已加入第 ${state.history.length} 局：${outcome === "B" ? "莊" : outcome === "P" ? "閒" : "和"}。Frozen V2 不結算上一筆、不更新 A/b；按「開始分析」重新計算。`);
    save();
    render();
  }
  ...
  function startAnalysis() {
    ...
    // Immutable inference only:
    // no bootstrap, no walk-forward, no replay, no settlement, no A/b update, no decay.
    state.active = true;
    state.lastPrediction = choose(state.history);
    setMessage(`Frozen V2 已預測第 ${state.history.length + 1} 局。四專家 Soft Fusion；A/b 固定、updates=0。`);
    save();
    render();
  }
  ...
  function endAnalysis() {
    ...
    state.active = false;
    state.lastPrediction = null;
    setMessage("分析已結束。歷史保留；Frozen V2 參數完全未修改。");
    save();
    render();
  }
  ...
  function backOne() {
    ...
    state.active = false;
    state.lastPrediction = null;
    setMessage(`已返回上一局（移除 ${removed}）。Frozen V2 沒有任何參數更新。`);
    save();
    render();
  }
  ...
  function renderDebug() {
    ...
    const debug = {
      model: "128D Frozen V2 Multi-Expert Contextual Arms",
      version: MODEL_VERSION,
      prior_source: PRIOR_SOURCE,
      pretrained_from_dataset: false,
      note: "Engineered immutable test model; repository still contains no Train/Validation/Test shoe dataset.",
      dimensions: { shoe: SHOE_DIM, road: ROAD_DIM, total: DIM },
      directional_windows: [4, 8, 16, 32],
      immutable_runtime: {
        bootstrap: false,
        walk_forward: false,
        replay: false,
      ...
      prediction: p ? {
        ...
        gap: p.gap,
        scores: p.scores,
        context_health: p.health,
        expert_weights: p.experts.weights,
        expert_scores: {
          trend: p.experts.trend,
          reversal: p.experts.reversal,
          derived_candidate: p.experts.derived.score,
          divergence: p.experts.divergence,
          fused: p.experts.fused
        },
        regime_consensus: p.experts.regime,
        derived_candidate: p.experts.derived,
        ratios: p.experts.ratios,
  ...
  function render() {
    el("modePill").textContent = state.active ? "Frozen V2 已預測" : "準備歷史";
    el("roundPill").textContent = `${state.history.length} 局`;
    el("brainState").textContent = "Frozen V2 · 4 Experts";
  ...
  if (migratedFromLegacy) {
    setMessage("已保留舊版本歷史；舊 A/b 完全未沿用。現在使用 Frozen V2 四專家 Soft Fusion。", true);
    save();
  }
